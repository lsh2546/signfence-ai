...
export function validateContract(contract: ContractData, documentChanged = false): CheckResult[] {
  ...
  const copyrightConditionPreserved = /only\s+after\s+(?:the\s+)?full\s+contract\s+amount\s+has\s+been\s+paid/i.test(contract.copyrightTransferCondition) || /계약\s*대금\s*전액.*지급\s*(?:한|된)?\s*후/.test(contract.copyrightTransferCondition);
  return [
    ...
    { id: 'required-clauses', label: 'Required clauses', passed: ['scope', 'payment', 'delivery', 'copyright'].every((clause) => contract.requiredClauses.includes(clause)) && copyrightConditionPreserved, detail: 'Scope, payment, delivery, and copyright transfer only after the full contract amount has been paid are required.' },
